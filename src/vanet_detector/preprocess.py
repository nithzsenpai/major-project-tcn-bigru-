...
def prepare_dataset(
    ...
    manifest = {
        ...
        "scenarios": sorted({scenario for _, scenario, _ in archives}),
        "attack_types": sorted(
            attack
            for attack in ATTACK_TO_CLASS
            if any(path.stem.endswith(f"_{attack}") for path, _, _ in archives)
        ),
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        nargs="+",
        choices=SCENARIOS,
        help="Optional scenario filter, for example highway_2 urban_2",
    )
    return parser
