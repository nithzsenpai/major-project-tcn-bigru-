...
def _add_temporal_features(group: pd.DataFrame) -> pd.DataFrame:
    ...
    # Pandas 3 / Copy-on-Write may expose a read-only NumPy view on Windows.
    # We modify missing positions below, so explicitly request a writable copy.
    rssi_delta = (
        rssi_filled_for_rolling.diff()
        .fillna(0.0)
