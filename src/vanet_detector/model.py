...
class VANETDetector(nn.Module):
    """Two-branch detector with flat or hierarchical three-class prediction."""
    ...
    def __init__(
        ...
        super().__init__()
        if classification_mode not in {"flat", "hierarchical"}:
        ...
        if classification_mode == "flat":
            ...
            if num_classes != 3:
                raise ValueError("Hierarchical prediction currently requires exactly 3 classes")
            self.classifier = None
            self.attack_classifier = nn.Linear(fusion_hidden // 2, 2)
            self.attack_type_classifier = nn.Linear(fusion_hidden // 2, 2)
    ...
    def _classification_outputs(self, fused: torch.Tensor) -> dict[str, torch.Tensor]:
        if self.classification_mode == "flat":
            assert self.classifier is not None
            return {"logits": self.classifier(fused)}

        assert self.attack_classifier is not None
        assert self.attack_type_classifier is not None
        attack_logits = self.attack_classifier(fused)
        attack_type_logits = self.attack_type_classifier(fused)
        attack_log_probabilities = F.log_softmax(attack_logits, dim=-1)
        type_log_probabilities = F.log_softmax(attack_type_logits, dim=-1)
        class_log_probabilities = torch.stack(
            (
                attack_log_probabilities[:, 0],
                attack_log_probabilities[:, 1] + type_log_probabilities[:, 0],
                attack_log_probabilities[:, 1] + type_log_probabilities[:, 1],
            ),
            dim=-1,
        )
        return {
            "logits": class_log_probabilities,
    ...
    def forward(self, behavior: torch.Tensor, relation: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        return self._classification_outputs(fused) | {
...
def build_model(config: dict, behavior_features: int, relation_features: int) -> VANETDetector:
    ...
    return VANETDetector(
        ...
        classification_mode=str(model_config.get("classification_mode", "flat")),
    )
