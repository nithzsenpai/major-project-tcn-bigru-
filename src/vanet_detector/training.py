...
def train(
    ...
    balanced_sampling = bool(training_config.get("balanced_sampling", False))
    use_class_weights = bool(training_config.get("class_weighting", True))
    if balanced_sampling and use_class_weights:
        raise ValueError(
            "Choose either class_weighting or balanced_sampling, not both. "
            "Using both can over-correct rare classes and increase false positives."
        )
    ...
    if evaluate_test:
        ...
        test_loader = _build_loader(
            test_dataset, batch_size, workers, False, False, pin_memory
        )
    ...
    final_results: dict[str, object] = {
        ...
        "test_evaluated_during_training": evaluate_test,
    }
    if evaluate_test:
    ...
    checkpoint.update(final_results)
    ...
    if evaluate_test:
        print(f"Test macro-F1: {final_results['test_metrics']['macro_f1']:.4f}")
    else:
        print("Test evaluation skipped; use evaluate.py only after model selection")
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--skip-test",
        action="store_true",
        help="Train and calibrate on Train/Validation without touching Test",
    )
    return parser
...
def main() -> None:
    ...
    train(config, run_dir, overwrite=args.overwrite, evaluate_test=not args.skip_test)
