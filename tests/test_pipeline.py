...
class PipelineTests(unittest.TestCase):
    ...
    def test_hierarchical_model_outputs_valid_class_probabilities(self) -> None:
        model = VANETDetector(
            behavior_features=len(BEHAVIOR_FEATURES),
            relation_features=len(RELATION_FEATURES),
            tcn_channels=[16, 16],
        ...
        self.assertEqual(tuple(output["logits"].shape), (3, 3))
        self.assertEqual(tuple(output["attack_logits"].shape), (3, 2))
        self.assertEqual(tuple(output["attack_type_logits"].shape), (3, 2))
        torch.testing.assert_close(
            torch.exp(output["logits"]).sum(dim=1), torch.ones(3)
        )
